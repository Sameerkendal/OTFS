...
%   'QAM'  --- > Independent FIR Filt. branch, ignores M, N, CP, etc.
%
...
ChannelCodingList     = ["Conv","Polar"];   % "Conv","Polar", "None"
DetectionList         = ["LMMSE"]; % "LMMSE", "MRC", "SP_LMMSE"

Check_FIR_Matrix_Equivalence = true;
Check_QAM_Equivalence   = true;
...
% Main sweep
% =========================
for Ch_out = Ch_outList

    for Modulation = ModulationList
        for Variant = VariantList
            for ChannelEstimation = ChannelEstimationList

                ChannelCodingLoop = ChannelCodingList;
                DetectionLoop = DetectionList;

                if strcmp(ChannelEstimation, "SP")
                    allowedSPCoding = ["Conv","Polar","None"];
                    keepMask = ismember(ChannelCodingLoop, allowedSPCoding);
                    ChannelCodingLoop = ChannelCodingLoop(keepMask);

                    if isempty(ChannelCodingLoop)
                        ChannelCodingLoop = allowedSPCoding;
                    end

                    DetectionLoop = "SP_LMMSE";
                    disp('SP mode selected: allowing only ChannelCoding = ["Conv","Polar","None"] and Detection = "SP_LMMSE".');
                end
                ...
                for ChannelCoding = ChannelCodingLoop
                    for Detection = DetectionLoop
                        ...
                        % QAM branch is only a standalone sanity check.
                        % Avoid duplicate QAM curves when Ch_out sweeps over Matrix/FIR Filt.
                        if strcmp(Modulation, 'QAM')
                            if ~strcmp(Ch_out, Ch_outList(1)) || ...
                               ~strcmp(Variant, VariantList(1)) || ...
                               ~strcmp(ChannelEstimation, ChannelEstimationList(1)) || ...
                               ~strcmp(ChannelCoding, ChannelCodingLoop(1)) || ...
                               ~strcmp(Detection, DetectionLoop(1))
                                continue;
                            end
                        end
                        ...
                        for iesn0 = 1:length(SNR_dB)
                            ...
                            for a = 1:length(seedGen)
                                ...
                                for ifram = 1:N_fram
                                    ...
                                    % Reproducible frame-level randomness.
                                    % This helps Matrix and FIR Filt. curves see the same data/noise sequence.
                                    frameSeed = 100000000 + 1000000*iesn0 + 1000*a + ifram;
                                    rng(frameSeed);
                                    ...
                                    last_fir_matrix_rel_error = NaN;
                                    last_qam_rel_error = NaN;

                                    %% ============================================
                                    % QAM INDEPENDENT FLOW BYPASS
                                    % FIR Filt., NO M/N, NO CP
                                    % ============================================
                                    if strcmp(Modulation, 'QAM')

                                        L_qam = 256;   % Change this to any arbitrary sequence length
                                        N_bits_qam = L_qam * M_bits;

                                        trans_info_bit = randi([0,1], N_bits_qam, 1);

                                        s_qam = qammod(trans_info_bit, M_mod, 'gray', 'InputType', 'bit');
                                        ...
                                        taps_qam = length(delays);
                                        delay_taps_qam = round(delays * Fs);
                                        Lmax_qam = max(delay_taps_qam);

                                        rng(frameSeed + 101);
                                        rayleigh_part = sqrt(pow_prof) .* ...
                                            (sqrt(1/2) * (randn(1, taps_qam) + 1i*randn(1, taps_qam)));
                                        g_i_qam = rayleigh_part;
                                        ...
                                        rng(frameSeed + 202);
                                        k_i_qam = Doppler_vel * cos(2*pi*rand(1, taps_qam));

                                        r_qam_fir = zeros(L_qam + Lmax_qam, 1);
                                        H_qam = zeros(L_qam + Lmax_qam, L_qam);

                                        for i = 1:taps_qam
                                            ell = delay_taps_qam(i);
                                            f_d = k_i_qam(i);

                                            for q = 0:(L_qam - 1)
                                                phase_comp = exp(1i * 2 * pi * f_d * q * Ts);
                                                tap_val = g_i_qam(i) * phase_comp;

                                                r_qam_fir(q + ell + 1) = r_qam_fir(q + ell + 1) + ...
                                                    tap_val * s_qam(q + 1);

                                                H_qam(q + ell + 1, q + 1) = H_qam(q + ell + 1, q + 1) + ...
                                                    tap_val;
                                            end
                                        end

                                        if Check_QAM_Equivalence
                                            r_qam_mat = H_qam * s_qam;

                                            qam_abs_error = norm(r_qam_fir - r_qam_mat);
                                            qam_rel_error = qam_abs_error / max(norm(r_qam_mat), eps);
                                            last_qam_rel_error = qam_rel_error;

                                            if qam_rel_error > CheckTolerance
                                                warning('QAM FIR and matrix outputs differ. Rel error = %.6e', qam_rel_error);
                                            end
                                        end

                                        noiseVar_qam = sigma_2_orig(iesn0);
                                        noise_qam = sqrt(noiseVar_qam/2) * ...
                                            (randn(size(r_qam_fir)) + 1i*randn(size(r_qam_fir)));

                                        r_qam = r_qam_fir + noise_qam;

                                        Es = abs(eng_sqrt)^2;

                                        s_hat_qam = (H_qam' * H_qam + (noiseVar_qam / Es) * eye(L_qam)) \ ...
                                            (H_qam' * r_qam);

                                        decoded_Bits = qamdemod(s_hat_qam, M_mod, 'gray', 'OutputType', 'bit');

                                        errors = sum(decoded_Bits ~= trans_info_bit);

                                        err_ber(iesn0) = err_ber(iesn0) + errors;
                                        avg_ber(iesn0) = err_ber(iesn0) / N_bits_qam / frameCounter;
                                        ...
                                        nmse_sum(iesn0) = nmse_sum(iesn0) + 0;
                                        avg_nmse(iesn0) = nmse_sum(iesn0) / frameCounter;

                                        if iesn0 == 1 && a == 1 && ifram == 1
                                            fprintf('QAM FIR vs Matrix relative error = %.6e\n', last_qam_rel_error);
                                        end
                                    ...
                                    switch Ch_out
                                        ...
                                        case "FIR Filt."
                                            ...
                                            for q = 0:(L_in - 1)
                                                ...
                                                for i = 1:taps
                                                    ell = l_i_true(i);
                                                    physical_phase = exp(1i * 2 * pi * f_d_taps_exact(i) * (q_eff - ell) / Fs);
                                            ...
                                            if Check_FIR_Matrix_Equivalence
                                                r_matrix_clean = G_true * s;

                                                fir_matrix_abs_error = norm(r_clean - r_matrix_clean);
                                                fir_matrix_rel_error = fir_matrix_abs_error / max(norm(r_matrix_clean), eps);
                                                last_fir_matrix_rel_error = fir_matrix_rel_error;

                                                if fir_matrix_rel_error > CheckTolerance
                                                    warning('FIR Filt. and Matrix clean outputs differ. Rel error = %.6e', fir_matrix_rel_error);
                                                end
                                            end
                                    ...
                                    switch ChannelEstimation
                                        ...
                                        case "Estimator"
                                            ...
                                            f_d_est_exact = k_i_est * Doppler_resolution;

                                            for q = 0:MN-1
                                                for i = 1:taps_est
                                                    physical_phase = exp(1i * 2 * pi * f_d_est_exact(i) * (q - l_i_est(i)) / Fs);
                                                    gs_est(l_i_est(i)+1, q+1) = gs_est(l_i_est(i)+1, q+1) + ...
                                                        g_i_est(i) * physical_phase;
                                                end
                                            end
                                        ...
                                        case "SP"
                                            ...
                                            [A_pilot, Gbasis] = build_sp_regression_matrix( ...
                                                N, M, l_i_true, k_i_true, s_pilot, Variant, length_cp, delta_f, T);
                                            ...
                                            for it = 1:numOuterIter
                                                ...
                                                [A_ref, Gbasis] = build_sp_regression_matrix( ...
                                                    N, M, l_i_true, k_i_true, s_ref, Variant, length_cp, delta_f, T);
                                    ...
                                    if ~strcmp(ChannelEstimation, "SP")
                                        ...
                                        if strcmp(Detection, "MRC")
                                            ...
                                            switch Variant
                                                case 'RCP'
                                                    [nu_ml_tilda,~,K_ml]=Gen_DT_and_DD_channel_vectors(N,M,Lmax,gs_used);
                                    ...
                                    fprintf('ChOut = %s | Modulation = %s | Variant = %s | ChannelEstimation = %s | Detection = %s | Coding = %s\n', ...
                                        char(Ch_out), char(Modulation), char(Variant), char(ChannelEstimation), char(Detection), char(ChannelCoding));

                                    if strcmp(ChannelEstimation, "SP")
                                        fprintf('SP params: Pd = %.2f, Pp = %.2f, numOuterIter = %d\n', Pd, Pp, numOuterIter);
                                    end

                                    if strcmp(Ch_out, "FIR Filt.") && ~isnan(last_fir_matrix_rel_error)
                                        fprintf('FIR Filt./Matrix clean-output relative error = %.6e\n', last_fir_matrix_rel_error);
                                    end
                        ...
                        if strcmp(Modulation, 'QAM')
                            curveLabels{curveCount} = 'QAM | Independent FIR Filt. | LMMSE | Variable Length';
                        else
                            curveLabels{curveCount} = sprintf('%s | %s | %s | %s | ChOut=%s', ...
                                char(Modulation), char(ChannelCoding), char(Detection), ...
                                char(ChannelEstimation), char(Ch_out));
                        end
                    end
                end
            end
...
function [A, Gbasis] = build_sp_regression_matrix(N, M, l_i, k_i, s_ref, Variant, length_cp, delta_f, T)
    P = length(l_i);
    MN = N*M;
    ...
    for p = 1:P
        Gbasis{p} = build_single_tap_G(N, M, l_i(p), k_i(p), Variant, length_cp, delta_f, T);
        A(:, p) = Gbasis{p} * s_ref;
    end
end

function Gp = build_single_tap_G(N, M, ell, k , Variant, length_cp, delta_f, T)
    MN = N*M;
    Gp = zeros(MN, MN);
    
    Fs = M * delta_f;
    Doppler_resolution = 1 / (N * T);
    f_d = k * Doppler_resolution;

    switch Variant

        case 'RCP'
            for q = 0:MN-1
                Gp(q+1, mod(q-ell, MN)+1) = exp(1i * 2 * pi * f_d * (q - ell) / Fs);
            end
        ...
        case 'CP'
            for n = 0:N-1
                for m = 0:M-1
                    ...
                    Gp(q+1, col+1) = exp(1i * 2 * pi * f_d * (q_eff - ell) / Fs);
