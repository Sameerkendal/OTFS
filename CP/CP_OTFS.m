...
ChannelCoding = "None";
Detection = ["TDLMMSE"];
for ChannelCoding   = ["Conv","Polar","None"]%[240,256,288,320,352,384]
    for max_speed = [12000] 
...
%% ENCODING %%%%%%
                    ...
                    elseif(strcmp(ChannelCoding,"Polar"))
                            ...
                            K = length(mssgBits);                    
                            crcLen = 24;  
                            msgCRC = nrCRCEncode(mssgBits, '24B');   
                            K_crc = length(msgCRC);
                            ...
                            modIn = nrRateMatchPolar(enc, length(msgCRC), E, false);  
                    ...
                    % elseif(strcmp(ChannelCoding,"LDPC"))
                    % 
                    %         trblklen = 256;           
                    %         K = trblklen;             
                    %         crcLen = 24;              
                    ...
                    % 
                    % 
                    %         msgCRC = nrCRCEncode(msgBits, '24B');      
                    % 
                    % 
                    %         if length(msgCRC) <= 384
                    %             baseGraph = 2;  
                    %         else
                    %             baseGraph = 1;   
                    %         end
                    % 
                    %         encBits = nrLDPCEncode(msgCRC, baseGraph);
                    %         codedLen =length(encBits);
                    %         rv = 0;
                    %         modulation = "QPSK";  
                    %         nLayers = 1;
                    % 
                    %         outBits = nrRateMatchLDPC(encBits, E, rv, modulation, nLayers);
                    % 
                    %         mappedBits = reshape(outBits, M_bits, []);
                    %         data = qammod(mappedBits, M_mod, "gray", "InputType", "bit");
                    % 
                    %         X = Generate_2D_data_grid(N, M, data, data_grid);
                    % 
...
%% Modulation %%%%
                    ...
                    if(strcmp(Modulation,"OTFS"))   %%OTFS
                        X_tilda=X*Fn'; 
                    elseif(strcmp(Modulation,"OFDM")) %%OFDM
                        X_tilda = Fm'*X;
                    end
...
%% CHANNEL CONSTRUCTION %%%% 
                    ...
                    for n=0:N-1
                        for m=0:M-1
                            for ell=0:Lmax
                                G_cp(m+n*M+1,n*M+mod(m-ell,M)+1)=gs(ell+1,m+n*(M+length_cp)+1);
                            end
                        end
                    end
...
%% Channel Reconstruction
                        ...
                        gs_est = zeros(delay_spread+1, N*(M+length_cp));
                   ...
                   %% NOISY CHANNEL MODEL %%%
                            ...
                            gs_hat = zeros(Lmax+1, N*(M+length_cp));
                            z = exp(1i*2*pi/(N*(M)));
...
%% Channel Vector Estimation %%%%%
                   ...
                   [nu_ml_tilda]=Gen_delay_time_channel_vectors(N,M,Lmax,gs,length_cp);   %DELAY TIME CHANNEL
                   [H_tf]=Generate_time_frequency_channel_CP(N,M,gs,L_set,length_cp);     %FREQUENCY TIME CHANNEL
                    
%% DETECTION
                    if(strcmp(Detection,"TDMRC"))
                        n_ite_MRC=100; 
                        omega=1;
                        if(M_mod==64)
                            omega=0.25;     
                        ...
                        decision=0;         
                        init_estimate=0;    
                        [estBits,det_iters_MRC,estdata] = MRC_delay_time_detector(N,M,M,M_mod,sigma_2(iesn0),data_grid,r,H_tf,nu_ml_tilda,L_set,omega,decision,init_estimate,n_ite_MRC);               
                    


                    elseif(strcmp(Detection,"TDLMMSE"))
                        if(strcmp(channel,"AWGN"))
                            estBits = reshape(qamdemod(Y,M_mod,"gray","OutputType","bit"),N_bits_perfram,1);
                        else
                         [estBits,estdata] = Block_LMMSE_detector(N,M,M_mod,sigma_2(iesn0),data_grid,r,gs,L_set,length_cp,1,Modulation);
                        end
                    elseif(strcmp(Detection,"FreqLMMSE"))
                        H = H_tf.';
                        
                        W = conj(H) ./ (abs(H).^2+ sigma_2(iesn0));
                        X_hat = W .* Y;
                        x_est=reshape(X_hat,1,N*M);

                        estBits=reshape(qamdemod(x_est,M_mod,'gray','OutputType','bit'),N_bits_perfram,1);
...
%% DECODING
                    ...
                    % 
                    % 
                    %             dummyEnc = nrLDPCEncode(zeros(K_crc,1), baseGraph);
                    %             codedLen = length(dummyEnc); 
                    %             R = K_crc / codedLen;
                    % 
                    %        recBits = nrRateRecoverLDPC(rxSoft, codedLen, R, rv, modulation, nLayers);
                    %        decLDPC = nrLDPCDecode(recBits, baseGraph, 30);
                    % 
                    %         decoded_crc = nrCRCDecode(decLDPC, "24B");
                    %         decoded_Bits = decoded_crc(1:K_msg);
 ...
 %% DISP error performance details   
                    ...
                    clc
                    disp('####################################################################') 
                    fprintf('CP-OTFS-(N,M,QAM size)');disp([N,M,M_mod]);
                    fprintf('CP-OTFS - TWO TAP ')
                    display(SNR_dB,'SNR (dB)');
                    display(current_frame_number,'Number of frames');
                    display(avg_ber,'Average FER');        
                         
                end
            end
        end
        if(strcmp(Modulation,"OTFS"))
            semilogy(SNR_dB,avg_ber,LineWidth=1.5,LineStyle='-',Color=colors{d},Marker=marker{d});
            hold on;
            
        else
            semilogy(SNR_dB,avg_fer,LineWidth=1.5,LineStyle=':',Color=colors{d},Marker=marker{d});
            hold on;
        end
...
title("Performance with Different Coding schemes")
legend("OFDM : Convolutional","OFDM : Polar","OFDM : None")
%legend("Code Rate = 0.5 ","Code Rate = 0.75","None")
%legend("Uncoded Bits = 240","Uncoded Bits = 256"," Uncoded Bits = 288","Uncoded Bits = 320","Uncoded Bits = 352","Uncoded Bits = 384");
